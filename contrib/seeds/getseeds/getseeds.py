...
def confFile():
    ...
    if platform.system() == 'Windows':
        folder = os.path.join(os.path.join(os.environ['APPDATA'], 'bank'))
    else:
        if platform.system() == 'Darwin':
            folder = os.path.expanduser('~/Library/Application Support/bank/')
        else:
            folder = os.path.expanduser('~/.bank')

    return os.path.join(folder, 'bank.conf')

conf_path = confFile()
if not os.path.exists(conf_path):
    parser.error('''bank configuration file not found. Manually enter your RPC password.\r\n'''
        ...
        '''rpcpassword=%x\r\n'''
        '''\r\n'''
        '''Keep that password secret! After creating the file, restart bank.''' % (conf_path, random.randrange(2**128)))
...
for node in json.loads(data.decode()):
    ...
    try:
        ...
        if e.errno == errno.ECONNREFUSED:
            print ('Unable to communicate with bank RPC')
        break
